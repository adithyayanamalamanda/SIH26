...
function App() {
  ...
  else view = <><div className="mobile-progress-heading"><h2>Today&apos;s activities</h2><time>{date}</time></div><div className="action-grid"><button className="feature-card memory-card" onClick={() => setActiveView('games')}><span className="feature-icon" aria-hidden="true">🧠</span><span><small>Cognitive practice</small><strong>Memory Game</strong></span><b>›</b></button><button className="feature-card medicine-card" onClick={() => setActiveView('reminders')}><span className="feature-icon" aria-hidden="true">💊</span><span><small>Daily care</small><strong>Medicine</strong></span><b>›</b></button><button className="feature-card water-card" onClick={() => setActiveView('reminders')}><span className="feature-icon" aria-hidden="true">💧</span><span><small>Stay refreshed</small><strong>Water</strong></span><b>›</b></button><button className="feature-card appointment-card" onClick={() => setActiveView('reminders')}><span className="feature-icon" aria-hidden="true">🗓</span><span><small>Keep on track</small><strong>Appointment</strong></span><b>›</b></button></div><div className="lower-grid"><article className="info-card"><p className="card-label">Attention practice</p><h3>Notice what changes</h3><button className="text-action" onClick={() => setActiveView('attention')}>Start attention game <span>→</span></button></article><article className="info-card progress-card"><p className="card-label">My progress</p><strong className="metric">Local<span> history</span></strong><button className="text-action" onClick={() => setActiveView('progress')}>View progress <span>→</span></button></article></div></>
